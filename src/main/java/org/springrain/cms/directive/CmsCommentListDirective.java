...
public class CmsCommentListDirective extends AbstractCMSDirective {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		try {
			...
			comment.setSiteId(this.getSiteId(params));
			comment.setBusinessId(this.getBusinessId(params));
			commentList = cmsCommentService.findCommentList(comment.getSiteId(), comment,page );
		} catch (Exception e) {
			logger.error(e.getMessage(), e);
			commentList = new ArrayList<>();
		}

		env.setVariable(DirectiveUtils.OUT_LIST,
				DirectiveUtils.wrap(commentList));
