...
public class CmsLinkDirective extends AbstractCMSDirective {
	...
	public void execute(Environment env, Map params, TemplateModel[] loopVars,
		...
		Integer themeType=DirectiveUtils.getInt("themeType", params);
		if(modelType==null){
			body.render(env.getOut());
			return;
		}
		if(themeType==null){
			themeType=Enumerations.ThemeSiteType.mp.getType();
		}
		...
		if(cmsLink == null){
			try {
				...
				//此处默认查前台的LINK
				cmsLink = cmsLinkService.findLinkBySiteBusinessIdModelType(siteId, businessId,modelType,themeType);
			} catch (Exception e) {
