...
public class CmsLinkServiceImpl extends BaseSpringrainServiceImpl implements ICmsLinkService {
	...
	@Override
	public CmsLink findLinkBySiteBusinessIdModelType(String siteId, String bussinessId, Integer modelType,Integer themeType)
			throws Exception {
		if(StringUtils.isBlank(siteId)||StringUtils.isBlank(bussinessId)){
			return null;
		}
		String cacheKey="findLinkBySiteBusinessId_"+siteId+"_"+bussinessId+"_"+modelType+"_"+themeType;
		
		CmsLink link = null;//super.getByCache(siteId, cacheKey, CmsLink.class);
		if(link!=null){
			return link;
		}
		
		Finder finder = Finder.getSelectFinder(CmsLink.class).append(" WHERE siteId=:siteId and  businessId=:bussinessId and modelType=:modelType and themeType=:themeType and active=1 ");
		finder.setParam("siteId", siteId).setParam("bussinessId", bussinessId).setParam("modelType", modelType).setParam("themeType", themeType);
