...
public class DateUtils {
	...
	public static Date plusMonth(Date date, int amount) {
		return plusDate(date, Calendar.MONTH, amount);
	}
	public static Date plusDay(Date date, int amount) {
		return plusDate(date, Calendar.DATE, amount);
	}
	//获取本月第一天
	public static Date getMonthFirstDay(Date date) throws Exception{
		return convertString2Date(DateUtils.convertDate2String("yyyy-MM",date)+"-01"); 
	}
    //两个日期天数差
	public static int bTDays(Date d1,Date d2)throws Exception{
		long cha=d2.getTime()-d1.getTime();
		int days=(int)(cha/(24*60*60*1000));
		return days;
	}
	
	public static int bTYears(Date d1,Date d2)throws Exception{
		int days=bTDays(d1, d2);
		int years = days/365;
		return years;
	}
	 ...
	 * @return 上个月的第一天
	 */
	public static Date getBoferBeginDate(Calendar currentDate) throws Exception{
	...
	public static Date getFirstDayOfMonth(String month) throws ParseException {
		if (StringUtils.isNotBlank(month)) {
			month += "-01";
			return DateUtils.convertString2Date(month);
		} else {
			return DateUtils.formatDate(DateUtils.getMinDate());
		}
	}
}
