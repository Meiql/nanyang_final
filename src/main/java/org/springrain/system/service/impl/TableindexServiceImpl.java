...
public class TableindexServiceImpl extends BaseSpringrainServiceImpl implements ITableindexService {
	...
	@Override
	public String updateNewIdSix(Class clazz) throws Exception {
		if(clazz==null){
			return null;
		}
		...
		Integer maxIndex=tableindex.getMaxIndex();
		maxIndex=maxIndex+1;
		String _id = getSixNum(maxIndex);
		String newId=tableindex.getPrefix()+_id;
	...
	private String getSixNum(Integer index){
		
		 String result = "";  
	     result = String.format("%0" + 6 + "d", index );  
