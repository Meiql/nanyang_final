...
public class SMenuController extends BaseController {
	...
	public ReturnDatas listjson(HttpServletRequest request, Model model,
		...
		List<Menu> datas = userRoleMenuService.findMenuByUserIdAll(SessionUser
				.getUserId());
	...
	@ResponseBody
	public ReturnDatas saveorupdate(Menu menu, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ReturnDatas returnObject = ReturnDatas.getSuccessReturnDatas();
	...
	public ReturnDatas leftMenu() {
		...
		try {
			List<Menu> listMenu = userRoleMenuService
					.findMenuAndLeafByUserId(userId);
			return new ReturnDatas(ReturnDatas.SUCCESS, null, listMenu);

		} catch (Exception e) {
			logger.error("获取导航菜单异常", e);
			return new ReturnDatas(ReturnDatas.ERROR,
					MessageUtils.SELECT_WARING);
		}
