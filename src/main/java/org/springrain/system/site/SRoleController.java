...
public class SRoleController extends BaseController {
	...
	public ReturnDatas listjson(HttpServletRequest request, Model model,
		...
		List<Role> datas = roleService.findListDataByFinder(null, page,
				Role.class, role);
		if (!CollectionUtils.isEmpty(datas)) {
			for (Role r : datas) {
				...
				// 查询角色对应菜单
				List<Menu> lsmenu = userRoleMenuService.findMenuByRoleId(r
						.getId());
	...
	@ResponseBody
	public ReturnDatas saveorupdate(Role role, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ReturnDatas returnObject = ReturnDatas.getSuccessReturnDatas();
	...
	public List<Role> ajaxUser(HttpServletRequest request) throws Exception {
		...
		Finder finder = Finder.getSelectFinder(Role.class, "id,name").append(
				" WHERE roleType=1 and name like :name order by name asc ");
