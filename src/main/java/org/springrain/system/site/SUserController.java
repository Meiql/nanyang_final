package org.springrain.system.site;
...
public class SUserController extends BaseController {
	...
	@ResponseBody
	public ReturnDatas saveorupdate(User user, HttpServletRequest request,
			HttpServletResponse response) throws Exception {
		ReturnDatas returnObject = ReturnDatas.getSuccessReturnDatas();
		...
		try {
			...
			// 处理管理的部门 韩彦阳 开始
			String[] managerOrgNames = request
					.getParameterValues("managerOrgNames");
			String[] managerOrgIds = request
					.getParameterValues("managerOrgIds");
			...
			if (managerOrgNames != null && managerOrgIds != null
					&& managerOrgIds.length == managerOrgNames.length) {
			...
			if (StringUtils.isBlank(id)) {
				...
				if (StringUtils.isNotBlank(user.getAccount())) {
					// 账号加上siteId 根据当前登陆人账号进行截取
					String ucode = SessionUser.getUserCode();
					if (StringUtils.isNotBlank(ucode)) {
						int i = ucode.indexOf("-");
						if (i > 0) {
							String siteId = ucode.substring(0, i);
							user.setAccount(siteId + "-" + user.getAccount());
						}
					}
	...
	public List<User> ajaxUser(HttpServletRequest request) throws Exception {
		...
		Finder finder = Finder.getSelectFinder(User.class, "id,name").append(
				" WHERE account like :account order by account asc ");
	...
	@ResponseBody
	public ReturnDatas modifiySave(HttpServletRequest request,
			HttpServletResponse response, Model model) throws Exception {
