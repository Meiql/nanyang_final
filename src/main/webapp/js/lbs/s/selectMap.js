...
function initMap(domId){
	...
	qq.maps.event.addListener(map, "click", function (a) {
        ...
        //调用腾讯地图webservice api;接口用每天一万次配额限制
	    url3 = encodeURI("http://apis.map.qq.com/ws/geocoder/v1/?location=" + a.latLng.getLat() + "," + a.latLng.getLng() + "&key=BCLBZ-VYK6I-FZ5GJ-5PJBC-PX7ST-PZB3U&output=jsonp&&callback=?");
	    try{
	    	jQuery.ajax({
		    	url:url3,
		    	dataType:"json",
	...
	qq.maps.event.addDomListener(btnSearch, 'click', function () {
	    ...
	    url = encodeURI("http://apis.map.qq.com/ws/place/v1/search?keyword=" + value + "&boundary=region("+search_city+",0)&page_size=9&page_index=1&key=NS3BZ-CAOW3-CWI3L-YC332-JN7M5-RVBUH&output=jsonp&&callback=?");
